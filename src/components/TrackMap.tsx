...
import { carPositionAt, pctToViewBox } from "@/data/calibration";
...
export function TrackMap({ currentTime, violations, selectedId, onSelect }: TrackMapProps) {
  ...
  const car = pctToViewBox(pos.xPct, pos.yPct);
          ...
          {violations.map((v) => {
            ...
            return (
              ...
              >
                ...
                {selected && (
                  <circle r={30} fill="none" stroke={confidenceToken(v.confidence)} strokeWidth={3} />
                )}
              </g>
            );
          })}
